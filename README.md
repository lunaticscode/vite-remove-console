...
| -------------- | ------------- | ------- | ------------------------------------------------------------------------- |
| `removeLevels` | `LogLevels[]` | None    | Array of console levels to remove (e.g., `log`, `info`, `warn`, `error`). |

## How It Works

1. The plugin scans the source code during the Vite build process.
2. It applies a regular expression to match and remove specified `console` statements from non-`node_modules` files.
